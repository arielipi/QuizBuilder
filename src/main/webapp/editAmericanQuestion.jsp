...
<%
	int qindex = (Integer)request.getAttribute("qindex");
		...
		AmericanQuestion question = (AmericanQuestion)request.getAttribute("question");	
...
<script language="Javascript" type="text/javascript"> 
		...
		var counter = <%=question.getIncorrect_answers().size()%>;
		var rowCounter = <%=question.getIncorrect_answers().size()%> + 4;
	...
	function addQuestion() {
		...
		var name = "incorrect" + counter;
		var nameDiv = "incorrectDIV" + counter;
		
		newdiv.innerHTML = "<div class='form-group' id='" + nameDiv + "'>"
		...
		var divIncorrects = document.getElementById('incorrects');
		divIncorrects.appendChild(newdiv);
		rowCounter++;
	}
...
<body class="first">
	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-md-6">
				<h1><%=LanguagesSupporter.editQuestion(language)%>:
				</H1>
			</div>
				...
				<form class="form-horizontal" action="/EditQuestion"
					...
					<div class="form-group">
						...
						<div class="col-md-8" id="ifYesImage" style="display: none"
							align="center">
							<input type="hidden" id="has_image_question"
								name="has_image_question"
								value=<%=question.getHasImageQuestion()%>> <input
								class="form-control" id="image_question" type="file"
								name="image_question">
						</div>
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<input class="form-control" id="correct_answer" type="text"
								size="31" name="correct_answer"
								value="<%=question.getCorrect_answer()%>" data-step="3"
					...
					<div id="incorrects">
						...
						<%
							int i=1;
							for (String incorrect: question.getIncorrect_answers()){
								String name = "incorrect" + i;
								String nameDIV = "incorrectDIV" + i;
																												
								if(i==1){
						%>

						<div class="form-group" id="<%=nameDIV%>">
							<label class="col-md-3 control-label" data-step="4"
								data-intro="<%=LanguagesSupporter.editAIQStepFour(language)%>">
								<%=LanguagesSupporter.addQuestionIncorrectAnswer(language)%>:
							</label>

							<div class="col-md-9">
								<input class="form-control" id="<%=name%>" type="text"
									size="31" name="<%=name%>"
									value="<%=incorrect%>" data-step="3"
									data-intro="<%=LanguagesSupporter.editAIQStepThree(language)%>">
							</div>
						</div>
